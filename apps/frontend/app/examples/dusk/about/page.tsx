...
export default function DuskColophon() {
  return (
    <div>
      <section className="border-b border-[#e9e1d1]/12 px-5 py-8 md:px-8">
        <p className="text-xs tracking-[0.25em] text-[#e9e1d1]/40">FILE: 02 — COLOPHON</p>
      ...
      <section className="border-b border-[#e9e1d1]/12 px-5 py-6 md:px-8">
        <p className="mb-4 text-xs tracking-[0.25em] text-[#e9e1d1]/40">RECORD METADATA</p>
        <dl>
          {meta.map((row) => (
            <div
              key={row.key}
              className="grid grid-cols-[7rem_1fr] gap-3 border-b border-[#e9e1d1]/10 py-2.5 text-sm sm:grid-cols-[10rem_1fr]"
            >
              <dt className="text-xs tracking-[0.15em] text-[#a7b98f]">{row.key}</dt>
              <dd className="text-[#e9e1d1]/75">{row.value}</dd>
            </div>
          ))}
        </dl>
      </section>
      ...
      <section className="px-5 py-6 md:px-8">
        <p className="mb-4 text-xs tracking-[0.25em] text-[#e9e1d1]/40">OPERATING PRINCIPLES</p>
        <ol className="space-y-3">
          {principles.map((p, i) => (
            <li key={i} className="flex gap-3 text-sm leading-relaxed text-[#e9e1d1]/75">
              <span className="text-[#cf9089]">{String(i + 1).padStart(2, "0")}</span>
              <span>{p}</span>
            </li>
          ))}
        </ol>
      ...
      <section className="border-t border-[#e9e1d1]/12 px-5 py-8 md:px-8">
        <p className="font-[family-name:var(--font-dusk-script)] text-xl text-[#a7b98f]">
